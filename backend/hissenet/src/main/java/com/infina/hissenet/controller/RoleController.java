...
public class RoleController implements RoleControllerDoc{
    ...
    public ResponseEntity<ApiResponse<RoleResponse>> createRole(@Valid @RequestBody RoleCreateRequest dto) {
        ...
        ApiResponse<RoleResponse> response = ApiResponse.ok(MessageUtils.getMessage("role.created.successfully"), createdRole);
        return ResponseEntity.status(HttpStatus.CREATED).body(response);
    }

    @GetMapping("/{id}")
    public ApiResponse<RoleResponse> getRoleById(@PathVariable Long id) {
        return ApiResponse.ok(MessageUtils.getMessage("role.retrieved.successfully"), roleService.getRoleById(id));
    }

    @GetMapping("/name/{name}")
    public ApiResponse<RoleResponse> getRoleByName(@PathVariable String name) {
        return ApiResponse.ok(MessageUtils.getMessage("role.retrieved.successfully"), roleService.getRoleByName(name).get());
    }

    @GetMapping
    public ApiResponse<List<RoleResponse>> getAllRoles() {
        return ApiResponse.ok(MessageUtils.getMessage("role.all.retrieved.successfully"), roleService.getAllRoles());
    }

    @GetMapping("/page")
    public ApiResponse<Page<RoleResponse>> getAllRolesPaged(Pageable pageable) {
        return ApiResponse.ok(MessageUtils.getMessage("role.paged.retrieved.successfully"), roleService.getAllRoles(pageable));
    }

    @GetMapping("/active")
    public ApiResponse<List<RoleResponse>> getActiveRoles() {
        return ApiResponse.ok(MessageUtils.getMessage("role.active.retrieved.successfully"), roleService.getActiveRoles());
    }

    @GetMapping("/inactive")
    public ApiResponse<List<RoleResponse>> getInactiveRoles() {
        return ApiResponse.ok(MessageUtils.getMessage("role.inactive.retrieved.successfully"), roleService.getInactiveRoles());
    }
    ...
    public ApiResponse<List<RoleResponse>> searchRoles(
        ...
        return ApiResponse.ok(MessageUtils.getMessage("role.search.successfully"), roles);
    }

    @PutMapping("/{id}")
    public ApiResponse<RoleResponse> updateRole(@PathVariable Long id,
                                                @Valid @RequestBody RoleUpdateRequest dto) {
        return ApiResponse.ok(MessageUtils.getMessage("role.updated.successfully"), roleService.updateRole(id, dto));
    }

    @DeleteMapping("/{id}")
    public ApiResponse<Void> deleteRole(@PathVariable Long id) {
        roleService.deleteRole(id);
        return ApiResponse.ok(MessageUtils.getMessage("role.deleted.successfully"));
    }

    @PatchMapping("/{id}/activate")
    public ApiResponse<Void> activateRole(@PathVariable Long id) {
        roleService.activateRole(id);
        return ApiResponse.ok(MessageUtils.getMessage("role.activated.successfully"));
    }

    @PatchMapping("/{id}/deactivate")
    public ApiResponse<Void> deactivateRole(@PathVariable Long id) {
        roleService.deactivateRole(id);
        return ApiResponse.ok(MessageUtils.getMessage("role.deactivated.successfully"));
    }

    @GetMapping("/{id}/exists")
    public ApiResponse<Boolean> existsById(@PathVariable Long id) {
        boolean exists = roleService.existsById(id);
        return ApiResponse.ok(MessageUtils.getMessage("role.exists.checked.successfully"), exists);
    }
    ...
    public ApiResponse<Boolean> existsByName(@PathVariable String name) {
        ...
        return ApiResponse.ok(MessageUtils.getMessage("role.name.exists.checked.successfully"), exists);
    }
}